...
export default function BoswellRun() {
  ...
  const [voiceGuide, setVoiceGuide] = useState(`ISAACSON STYLE — quick rules
...
With the kids gathered around a beige desktop in the rental house, Barksdale typed a case number and watched the browser fetch a court decision in seconds. "Wow, how did it do that?" Susan asked. Betsy leaned in next; a lesson plan appeared as quickly as if it were on the hard drive. David wanted concert dates; a fan site listed a New Orleans show the next April. "It is not in the machine," Barksdale said. "All I have is a browser and a network." Newspapers from Tokyo and London, a virtual walk through the Louvre—the room filled with the sense that something fundamental had shifted. By the end of the night, what had seemed reckless began to feel inevitable.`);
  ...
  const nbPrompt1 = useMemo(
    () =>
      `\nBOSWELL RUN — STEP 1A (NotebookLM - Context + Chronology)
Knowledge Base: ${kb}
...
3) People & Roles — Cast of Characters relevant to this chapter.
   - For each person: Name; one-line role/identity; relationship to the chapter’s main subject; first appearance (approx date/age if known); why they matter in this chapter (1 line).
...
Output as clearly labeled sections.\n`,
  [kb, chapter, goal, notes]
);

  const nbPrompt2 = useMemo(
    () =>
      `\nBOSWELL RUN — STEP 1B (NotebookLM - Detailed Evidence)
Knowledge Base: ${kb}
Chapter: ${chapter || "(unspecified)"}
Goal: ${goal}
Context notes (optional):
...
Please produce:
1) Expanded report for each chronological turning point.
   - Sub-bullets with ALL relevant quotes.
   - Identify who said each quote, with source citation and estimated timing.
   - Include ALL key scenes if identifiable (characters, setting, source).
...
[VOICE GUIDE]`,
  ...
  const lastTail = (s: string, nChars = 1800) => (s.length <= nChars ? s : s.slice(s.length - nChars));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-black text-slate-100">
      <div className="max-w-6xl mx-auto px-4 py-10">
        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">Boswell — Run Console</h1>
        <p className="mt-2 opacity-80">Step 1: run NotebookLM → Step 2: assemble prompt → Step 3: run in Claude (API).</p>

        {/* tabs */}
        <div className="mt-6 flex gap-2">
          <button
            onClick={() => setTab("prep")}
            className={`px-3 py-2 rounded-2xl border ${tab === "prep" ? "bg-white/10" : "bg-white/5"} border-white/10`}
          >
            Step 1–2: Prep & Assemble
          </button>
          <button
            onClick={() => setTab("claude")}
            className={`px-3 py-2 rounded-2xl border ${tab === "claude" ? "bg-white/10" : "bg-white/5"} border-white/10`}
          >
            Step 3: Claude (API)
          </button>
        </div>
          ...
          <>
            <div className="mt-6 grid md:grid-cols-2 gap-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <label className="text-sm opacity-80">Knowledge Base</label>
                <select
                  className="w-full mt-1 p-3 rounded-xl bg-black/30 border border-white/10 focus:outline-none"
                  value={kb}
                ...
                <label className="text-sm opacity-80 mt-4 block">Chapter (feeds the ASSIGNMENT)</label>
                <input
                  className="w-full mt-1 p-3 rounded-xl bg-black/30 border border-white/10 focus:outline-none"
                ...
                <p className="text-xs opacity-70 mt-1">
                  This appears in <strong>ASSIGNMENT</strong> and <strong>SOURCE MATERIALS</strong>.
                </p>

                <label className="text-sm opacity-80 mt-4 block">Goal</label>
                <input
                  className="w-full mt-1 p-3 rounded-xl bg-black/30 border border-white/10 focus:outline-none"
                  value={goal}
                ...
                <label className="text-sm opacity-80 mt-4 block">Optional notes</label>
                <textarea
                  className="w-full mt-1 p-3 rounded-xl bg-black/30 border border-white/10"
                  rows={6}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                />

                <label className="text-sm opacity-80 mt-4 block">Voice Guide (Isaacson rules)</label>
                <textarea
                  className="w-full mt-1 p-3 rounded-xl bg-black/30 border border-white/10 text-xs"
                  rows={8}
                  value={voiceGuide}
                  onChange={(e) => setVoiceGuide(e.target.value)}
                />

                <label className="text-sm opacity-80 mt-4 block">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-4">
                <div>
                  <div className="flex items-center justify-between">
                    <div className="text-sm opacity-80">Step 1A — Context + Chronology (send to NotebookLM)</div>
                    <a href="https://notebooklm.google/" target="_blank" rel="noreferrer" className="text-xs underline opacity-90">
                      Open NotebookLM ↗
                    </a>
                  </div>
                  <textarea
                    readOnly
                    className="w-full mt-2 p-3 rounded-xl bg-black/30 border border-white/10 text-xs"
                    rows={8}
                    value={nbPrompt1}
                  />
                  <button onClick={() => copy(nbPrompt1)} className="mt-2 px-3 py-2 rounded-2xl bg-emerald-500/90 hover:bg-emerald-500">
                    Copy Step 1A Prompt
                  </button>
                </div>

                <div>
                  <div className="text-sm opacity-80">Step 1B — Detailed Evidence + Bibliography (send to NotebookLM)</div>
                  <textarea
                    readOnly
                    className="w-full mt-2 p-3 rounded-xl bg-black/30 border border-white/10 text-xs"
                    rows={8}
                    value={nbPrompt2}
                  />
                  <button onClick={() => copy(nbPrompt2)} className="mt-2 px-3 py-2 rounded-2xl bg-emerald-500/90 hover:bg-emerald-500">
                    Copy Step 1B Prompt
                  </button>
                </div>

                <div>
                  <div className="text-sm opacity-80">Paste NotebookLM Outputs Here</div>
                  <label className="text-xs opacity-70 mt-2 block">NB-LM Output — Step 1A (Context + Chronology)</label>
                  <textarea
                    className="w-full mt-1 p-3 rounded-xl bg-black/30 border border-white/10 text-xs"
                    rows={8}
                  ...
                  />
                  <label className="text-xs opacity-70 mt-3 block">NB-LM Output — Step 1B (Detailed Evidence + Bibliography)</label>
                  <textarea
                    className="w-full mt-1 p-3 rounded-xl bg-black/30 border border-white/10 text-xs"
                    rows={10}
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="flex items-center justify-between">
                <div className="text-sm opacity-80">Step 2 — Assemble & Copy Prompt for Claude</div>
                <div className="flex gap-3 items-center">
                  <button onClick={() => copy(claudeSystemHeader)} className="text-xs underline opacity-90">
                    Copy Header Only
                  </button>
                  <button onClick={() => copy(assembledClaudePrompt)} className="text-xs underline opacity-90">
                    Copy FULL Assembled Prompt
                  </button>
                  <button
                    onClick={runAndOpenClaudeTab}
                    className="px-3 py-2 rounded-2xl bg-indigo-500/90 hover:bg-indigo-500 disabled:opacity-40"
              ...
              <p className="mt-2 text-xs opacity-80">
                This is exactly what will be sent to Claude: header, Voice Guide, optional Style Exemplar, and both NotebookLM outputs.
              </p>
              <textarea
                readOnly
                className="w-full mt-2 p-3 rounded-xl bg-black/30 border border-white/10 text-xs"
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setTab("prep")} className="text-xs underline opacity-90">
                  ← Back to Prep
                </button>
                {!loadingClaude ? (
                  <button onClick={runClaude} className="text-xs underline opacity-90">
                    Re-run
                  </button>
                ) : (
                  <button onClick={stopClaude} className="text-xs underline opacity-90">
                    Stop
                  </button>
                )}
              </div>
            </div>
            <p className="mt-2 text-xs opacity-80">We auto-send the assembled prompt when this tab opens.</p>
            {claudeError && <div className="mt-3 text-xs text-rose-300">{claudeError}</div>}
            <textarea
              readOnly
              className="w-full mt-3 p-3 rounded-2xl bg-black/30 border border-white/10 text-xs"
              rows={22}
