...
[VOICE GUIDE]`,
  ...
  async function runClaude() {
    ...
    try {
      await streamOnce(
        ...
        (chunk) => setClaudeOutput((prev) => prev + chunk),
        controller.signal
      );
    } catch (e: any) {
  ...
  // ----- Clean, professional UI palette with navy/indigo theme -----
  const pageBg = isDark
    ? "bg-slate-950 text-slate-100"
    : "bg-gray-50 text-slate-900";
  const headerBg = isDark ? "bg-slate-900/95" : "bg-white/95";
  const border = isDark ? "border-slate-700" : "border-gray-300";
  const panel = isDark 
    ? "bg-slate-900 border border-slate-700" 
    : "bg-white border border-gray-200 shadow-sm";
  const input = isDark
    ? "bg-slate-800 border-slate-600 text-slate-100 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
    : "bg-white border-2 border-gray-300 text-slate-900 placeholder:text-gray-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20";
  ...
  const btnPrimary = "bg-slate-700 hover:bg-slate-800 text-white shadow-sm";
  const btnSecondary = isDark 
    ? "bg-slate-700 hover:bg-slate-600 border border-slate-600 text-slate-200" 
    : "bg-white hover:bg-gray-50 border-2 border-gray-300 text-gray-700 shadow-sm";

  // ===== SINGLE RETURN BELOW =====
  return (
    <div className={`min-h-screen ${pageBg} font-inter antialiased`} style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      {/* Clean header */}
      <div className={`sticky top-0 z-30 backdrop-blur-xl ${headerBg} border-b ${border}`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-slate-800 via-slate-600 to-slate-800 dark:from-slate-100 dark:via-slate-300 dark:to-slate-100 bg-clip-text text-transparent">
              Boswell Run Console
            </h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className={`mt-4 rounded-lg ${panel} p-6 animate-in slide-in-from-top-2 duration-300`}>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <h4 className="font-medium mb-1">Run NotebookLM Step 1A</h4>
                    <p className={`text-sm ${muted}`}>
                      Copy the "Step 1A" prompt and paste it into NotebookLM. This will generate a context summary, chronological outline, and cast of characters. Paste the result back here.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <h4 className="font-medium mb-1">Run NotebookLM Step 1B</h4>
                    <p className={`text-sm ${muted}`}>
                      Copy the "Step 1B" prompt and paste it into NotebookLM. This will gather detailed evidence, quotes, and scenes for each chronological turning point. Paste the result back here.
                    </p>
                  </div>
                </div>
              ...
              <div className={`mt-6 p-4 ${isDark ? 'bg-indigo-900/30 border-indigo-800' : 'bg-indigo-50 border-indigo-200'} rounded-lg border`}>
                ...
                <ul className={`text-sm ${muted} space-y-1`}>
                  ...
                  <li>• The more detailed your Step 1B output, the richer your final chapter will be</li>
        ...
        {tab === "prep" && (
          <>
            <div className="grid lg:grid-cols-2 gap-8">
                ...
                <div className="border-t border-gray-200 dark:border-slate-700 pt-6 mt-8">
                  ...
                  <div className="space-y-5">
                    <div>
                      ...
                      <textarea
                        className={`w-full p-3 rounded-lg ${input} text-sm font-mono transition-all`}
                        rows={10}
                        value={voiceGuide}
                    ...
                    <div>
                      <label className={`block text-sm font-medium ${subtle} mb-2`}>
                        Style Exemplar (Optional) - Paste in sample text with your preferred biographical style
                      </label>
                      <textarea
                        className={`w-full p-3 rounded-lg ${input} text-sm font-mono transition-all`}
                        rows={8}
              ...
              <div className={`rounded-lg ${panel} p-6`}>
                ...
                <div className="space-y-6">
                  <div>
                    <div className="mb-3">
                      <h3 className={`text-sm font-semibold ${subtle}`}>Step 1A: Context & Chronology</h3>
                      <p className={`text-xs ${muted} mt-1`}>Research prompt for NotebookLM</p>
                    </div>
                    <textarea
                      readOnly
                      className={`w-full p-3 rounded-lg ${input} text-xs font-mono`}
                      rows={8}
                      value={nbPrompt1}
                    />
                    <button 
                  ...
                  <div>
                    <div className="mb-3">
                      <h3 className={`text-sm font-semibold ${subtle}`}>Step 1B: Detailed Evidence</h3>
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className={`block text-xs font-medium ${muted} mb-2`}>
                          Step 1A Output ({nb1Counts.words.toLocaleString()} words)
                        </label>
                        <textarea
                          className={`w-full p-3 rounded-lg ${input} text-xs font-mono`}
                          rows={6}
                          placeholder="Paste the full Step 1A result from NotebookLM here..."
                          value={nbOut1}
                      ...
                      <div>
                        <label className={`block text-xs font-medium ${muted} mb-2`}>
                          Step 1B Output ({nb2Counts.words.toLocaleString()} words)
                        </label>
                        <textarea
                          className={`w-full p-3 rounded-lg ${input} text-xs font-mono`}
                          rows={8}
                          placeholder="Paste the full Step 1B result from NotebookLM here..."
                          value={nbOut2}
            ...
            <div className={`mt-8 rounded-lg ${panel} p-6`}>
              ...
              <textarea
                readOnly
                className={`w-full p-4 rounded-lg ${input} text-xs font-mono`}
                rows={16}
        ...
        {tab === "claude" && (
          <div className={`rounded-lg ${panel} p-6`}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-lg font-semibold">Claude Generation</h2>
                <p className={`text-sm ${muted} mt-1`}>
                  Using Claude 4.1 Opus • Streaming response
                </p>
              </div>
            ...
            <textarea
              ref={claudeRef}
              readOnly
              className={`w-full p-4 rounded-lg ${input} text-sm font-mono`}
              rows={24}
              value={loadingClaude && !claudeOutput ? "Claude is generating your chapter...\n\nThis may take a few minutes for a 7,000-word response." : claudeOutput}
              placeholder="Claude's response will stream here in real-time..."
            />
            
            <div className="flex items-center justify-between mt-4">
              <div className={`text-xs ${muted}`}>
                {claudeCounts.words.toLocaleString()} words • {claudeCounts.chars.toLocaleString()} characters
                {loadingClaude && <span className="ml-2 text-indigo-600">• Generating...</span>}
              </div>
              {claudeOutput && (
                <button 
